...
#    q = torch.clamp(q, -maxq, maxq)
#
#    # With symmetric quant, zero is always 0, so just return scale * q
#    return scale * q
...
class Quantizer(nn.Module):
    ...
    def find_params(self, x, weight=False):
        ...
        shape = x.shape
        #print("self.perchannel: ", self.perchannel)
        if self.perchannel:
            if weight:
                #print("before_weight_shape: ", x.shape)
                x = x.flatten(1)
                #print("weight_shape: ", x.shape)
                #print("inside weight if")
            else:
                #print("not inside weight if")
                if len(shape) == 4:
                    x = x.permute([1, 0, 2, 3])
                    x = x.flatten(1)
                if len(shape) == 3:
